...
export async function POST(request: Request) {
  ...
  const { error } = await createSupabaseAdminClient().from("usuarios").update({ numero_whatsapp: parsed.data.numero_whatsapp, numero_whatsapp_actualizado_en: new Date().toISOString() }).eq("id", user.id);
