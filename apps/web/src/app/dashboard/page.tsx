...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const monthIncomes = (incomes ?? []).filter((income) => income.fecha_ingreso >= selectedRange.start && income.fecha_ingreso <= selectedRange.end);
  const previousIncomes = (incomes ?? []).filter((income) => income.fecha_ingreso >= previousRange.start && income.fecha_ingreso <= previousRange.end);
  ...
  const monthIncomeTotal = monthIncomes.reduce((total, income) => total + Number(income.monto ?? 0), 0);
  const previousIncomeTotal = previousIncomes.reduce((total, income) => total + Number(income.monto ?? 0), 0);
  const balance = monthIncomeTotal - monthTotal;
  const previousBalance = previousIncomeTotal - previousTotal;
  const changePercentage = getChangePercentage(balance, previousBalance);
  ...
  return <main className="dashboard-shell">
    ...
    {!profile?.numero_whatsapp && <section className="clay-panel setup-panel"><div className="panel-icon" aria-hidden="true"><Link2 size={22} /></div><div><p className="eyebrow">Primer paso</p><h2>Conecta tu WhatsApp</h2><p className="muted">Vincula tu número de WhatsApp para empezar a registrar gastos.</p><WhatsappLinkForm /></div></section>}
