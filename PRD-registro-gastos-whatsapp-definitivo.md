...
**Alcance de la consola de soporte en v1:** una pantalla simple dentro del mismo dashboard Next.js, accesible solo a usuarios con rol `admin` (verificado por RLS/policy, no solo ocultando el enlace en el frontend). Permite: buscar una cuenta por número actual, iniciar el cambio de número con el nuevo número y forzar su verificación por OTP, y ver la bitácora de auditoría de cambios. No incluye edición directa de gastos ni gestión de usuarios más allá del cambio de número — cualquier otra corrección se hace vía Supabase Studio en v1.

## 6. Autenticación y vinculación

1. El usuario abre la web, ingresa su número en formato E.164 y solicita un OTP.
2. El sistema genera el código y lo envía por **WhatsApp usando la misma instancia de Evolution API** que opera el bot (no se contrata un proveedor adicional como Twilio). El código se genera en el backend, se persiste con expiración corta (ej. 5 minutos) y un máximo de intentos de verificación, y se envía como mensaje saliente a través del adaptador de Evolution API ya existente.
3. El usuario verifica el OTP en la web. Si es válido y no ha expirado, Supabase crea o autentica `auth.users`.

*Nota: al usar el mismo canal para OTP y para el bot, un corte de sesión de Evolution API afecta tanto el registro de usuarios nuevos como el flujo de gastos. Se acepta este riesgo compartido en v1 a cambio de no sumar un proveedor y costo adicional; si el volumen crece o la estabilidad de Evolution API es un problema recurrente, separar el canal de OTP es la primera mitigación a evaluar.*
...
| `corregir 25000`, `corregir categoría Transporte`, o cualquier corrección en lenguaje natural | Se interpreta vía IA (mismo contrato del §8), pasando el JSON del borrador pendiente como contexto junto con el mensaje. El modelo devuelve el JSON actualizado, se valida con el mismo esquema estricto, y se solicita nueva confirmación. |
...
**Reutilización para correcciones:** cuando el usuario responde con un comando `corregir` sobre un gasto activo, el mismo contrato se invoca pasando el JSON del borrador pendiente como contexto adicional (no solo el mensaje nuevo). El modelo devuelve el JSON completo actualizado, no un diff — así el orquestador siempre valida y persiste un objeto completo con el mismo esquema, sin lógica de parcheo adicional.
...
### 9.1 Infraestructura v1 (confirmado: free tier en todo)

| Componente                     | v1 — hasta 10 usuarios                                                                    | Camino a 1000 usuarios                                                                      |
| ------------------------------ | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Evolution API                  | VPS o capa gratuita (Railway/Fly.io free tier); una sola instancia alcanza para OTP + bot | Mismo software; se sube el tamaño del servidor si el volumen lo exige                       |
| n8n / orquestación             | n8n cloud, plan gratuito                                                                  | Self-hosted o plan pago cuando el volumen de ejecuciones se acerque al límite del free tier |
| Supabase Postgres/Auth/Storage | Plan free                                                                                 | Plan Pro cuando se acerque a límites de fila/egress/pausa por inactividad                   |
| Next.js + Vercel               | Plan Hobby                                                                                | Plan Hobby sigue alcanzando salvo tráfico inusualmente alto                                 |
| Modelo de IA                   | Recomiendame el mejor modelo de IA GRATIS                                                 | Sin cambios — el costo escala linealmente con uso                                           |

El esquema de datos, las políticas RLS, la lógica de estado conversacional y los contratos de IA/webhook no cambian al escalar — únicamente se ajustan planes y capacidad de infraestructura.
...
1. **Fundación de datos:** migraciones SQL, catálogo seed, trigger `auth.users → usuarios`, restricciones, RLS, rol `admin` y pruebas de aislamiento.
2. **Adaptador WhatsApp (se adelanta porque OTP depende de él):** VPS/Evolution en free tier, validación de webhook, normalización de eventos, idempotencia, envío de mensajes salientes y respuesta básica.
3. **Autenticación web:** generación y envío de OTP vía Evolution API, verificación con expiración/límite de intentos, sesión, registro, vinculación del número y dashboard vacío protegido.
...
6. **Correcciones y soporte:** comando `corregir` vía IA reutilizando el contrato del §8, auditoría, pantalla de soporte con rol `admin` y cambio seguro de número.
